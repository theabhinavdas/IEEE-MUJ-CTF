...
if (isset($_POST['flag-submit'])) {
   ...
   if (isset($_POST['flag'])) {
      ...
      if ($flag_text == "terminal" || $flag_text == "Terminal") {
         try {
            $conn = new PDO('mysql:host=theabhinavdas.ipagemysql.com;dbname=ctf_db', $dbuser, $dbpass);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("UPDATE ctf_users SET `level8` = 100 where email = :email");
            $stmt->execute(array('email' => $email));
            $_SESSION['is_auth_for_8'] = true;
            header("location: 9999.php");
  ...
  <body>
    ...
    <div class="jumbotron">
      ...
      <div class="container">
        ...
        <p>CLI (Command Line Interface) is another name for?</p>
        <p>
           Hint: It was the black-background application where I was running nmap. Start with a 't' .
        </p>
      </div>
    </div>
